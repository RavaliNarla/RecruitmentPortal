...
- Captured values are added to each form's submit payload under **different key names depending on the form** — `dynamicFieldValues` for Requisition (`createRequisitionMapper.js`, unaffected by anything below), and (for Job Posting) something that changed twice:
  1. *Originally*: `orgDynamicFieldValues` traveled as its own top-level key on the Job Posting DTO. **A bug was found and fixed at this stage**: a second new key, `applicableInclusionIds` (§12), was being silently dropped by `jobPositionCreateMapper.js`/`positionUpdate.mapper.js` because they didn't destructure/re-include it in their output — `AddPosition.jsx` built it correctly but it never reached the network request.
  2. *Then superseded entirely*: inspecting the backend's actual Swagger schema for the Job Posting DTO showed it only exposes a single `dynamicData` field (typed as a string), with **no `orgDynamicFieldValues` or `applicableInclusionIds` property at all** — meaning stage 1's top-level keys were being silently ignored by the backend the whole time, bug-fix or not, since the backend simply doesn't bind unrecognized properties. The real fix: both mappers now build `dynamicData: JSON.stringify({ orgDynamicFieldValues, applicableInclusionIds })` instead of sending either as its own key. The backend treats this as an opaque blob it just persists and returns as-is (confirmed with the SuperAdminPortal team — it doesn't validate/reshape the JSON inside), so the nesting structure is ours to choose freely; the legacy, unrelated per-position ad-hoc builder's `dynamicFields` key (§7.2) is untouched and still travels separately.
  3. On read (`AddPosition.jsx`'s edit-mode restore), `existingPosition.dynamicData` is `JSON.parse`'d once and both `orgDynamicFieldValues` and `applicableInclusionIds` are pulled from the parsed object — with a fallback to the old top-level `applicableInclusionIds` field for any position record saved before this change (pre-migration data).
...
- **`AddPosition.jsx`** — "Applicable Inclusions" section: checkbox per Active Inclusion in the org, backed by `useOrgInclusions()`. Selected ids (a plain array of Inclusion ids — numeric in practice, e.g. `1784293414104`, not uuids) are collected as `applicableInclusionIds` in component state, but **travel to the backend nested inside `dynamicData`** (`jobPositionCreateMapper.js` / `positionUpdate.mapper.js` — see §10 for why: the Job Posting DTO has no top-level `applicableInclusionIds` property, only `dynamicData`).
...
7. **The Job Posting backend DTO only recognizes the exact top-level fields in its Swagger schema — anything else is silently ignored, not an error.** `orgDynamicFieldValues` and `applicableInclusionIds` both learned this the hard way (§10, §12): neither is a real DTO property, so both now travel nested inside `dynamicData` (`JSON.stringify({...})`), the one field the backend actually persists as an opaque blob. Before adding any new payload key to `AddPosition.jsx`, check the DTO's real schema first — if it's not listed there, it needs to go inside `dynamicData`, not added as a new top-level key (which would compile fine, submit fine, and just vanish server-side).
